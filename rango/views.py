...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        # because .get() returns None if the value does not exist, while [] raises KeyError.
        username = request.POST.get('username')
        password = request.POST.get('password')

        # Use Django's authentication system to verify the username/password combination.
        ...
        if user:
            ...
            if user.is_active:
                # Account is valid and active, log the user in.
                login(request, user)
                return redirect(reverse('rango:index'))
            else:
                # Inactive account was used - no login.
        ...
        # The request is not a POST (most likely GET), so display the login form.
